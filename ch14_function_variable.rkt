...
;; Arithmetic Expression with Identifiers and Functions and Variables
(define-type VCFAE
  [num (n number?)]
  [add (lhs VCFAE?) (rhs VCFAE?)]
  [id (name symbol?)]
  [if0 (test VCFAE?) (ontrue VCFAE?) (onfalse VCFAE?)]
  [fun (param symbol?) (body VCFAE?)]
  [app (fun-expr VCFAE?) (arg-expr VCFAE?)]
  [mutate (var symbol?) (value-expr VCFAE?)]
...
(define-type Store
  [mtSto]
  [aSto (location number?) (value VCFAE-Value?) (store Store?)])

;; value of VCFAE expressions
(define-type VCFAE-Value
  [numV (n number?)]
  [closureV (param symbol?) (body VCFAE?) (env Env?)])

;; preprocess: sexp -> VCFAE
